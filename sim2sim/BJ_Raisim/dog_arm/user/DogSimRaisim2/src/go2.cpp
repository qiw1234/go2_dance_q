...
int main(int argc, char* argv[]) {
    ...
    world.setTimeStep(dt_i*dt); //500Hz
    last_vel.setZero();
    ...
    // 左前右前左后右后
    jointNominalConfig << 0, 0, 0.075, quat_d(0), quat_d(1), quat_d(2), quat_d(3),0.35, 1.36, -2.65, -0.35, 1.36, -2.65, 0.5, 1.36, -2.65, -0.5, 1.36, -2.65;//go2
    jointDgain.tail(dof_leg).setConstant(1.0);
    ...
    float loop_time = (float)dt_i * dt * 1000;//ms
  ...
  for ( ; ; ) {
    ...
    if(sim_start==1)
    {
        ...
        sim_io(dt_i*dt);  //1.0e-03
    }
    t+=dt_i;
    clock_gettime(CLOCK_REALTIME, &time2);
    timesim = (float)(time2.tv_sec - time1.tv_sec)*1000.0 + (float)(time2.tv_nsec - time1.tv_nsec)/1000000.0;//ms
