...
raisim::ArticulatedSystem *panda5;
...
const char* datapath= "/home/ixxuan/Project/Tsinghua/Apr18_16-57-48_/raisim.txt";
...
void sim_io(float dt)
{
    ...
    for(int i=0;i<7;i++)
    {
        _sharedMemory().simToRobot.spiData.q_arm[i] = gc(0 + 3*i);
    }
#endif
    float target_tau_new[12];
    for (int leg = 0; leg < 4; leg++) {
      for (int joint = 0; joint < 3; joint++) {
    //    target_tau_new[leg * 3 + joint] = _actuatorModels[joint].getTorque(
    //        tau_now[leg * 3 + joint],
    //        gv[leg * 3 + joint]);
          target_tau_new[leg * 3 + joint] = tau_now[leg * 3 + joint];
      }
    }
    for(int i = 0;i<4;i++)
    {
        _sharedMemory().simToRobot.spiData.tau_abad[i] = target_tau_new[0 + 3*i];
        _sharedMemory().simToRobot.spiData.tau_hip[i] = target_tau_new[1 + 3*i];
        _sharedMemory().simToRobot.spiData.tau_knee[i] = target_tau_new[2 + 3*i];
    }
    ...
    Mat3<float> rt;
    panda5->getFramePosition(5, body_frame_p);
    panda5->getFrameVelocity(5, body_frame_v);
    panda5->getFrameOrientation(panda5->getFrameIdxByName("imu_joint"), body_frame_orient);
    panda5->getFrameAngularVelocity(5, body_frame_omega);
    ...
    int num_dof = 18;//18;
    //joint PD controller
    Eigen::VectorXd jointNominalConfig(num_dof+1), jointVelocityTarget(num_dof);
    Eigen::VectorXd jointState(num_dof), jointForce(num_dof), jointPgain(num_dof),
        jointDgain(num_dof);
    raisim::VecDyn jointForceFeed(num_dof);
    jointPgain.setZero();
    jointDgain.setZero();
    ...
    float target_q[12] = {_sharedMemory().robotToSim.spiCommand.q_des_abad[0],
    ...
    float target_qd[12] = {_sharedMemory().robotToSim.spiCommand.qd_des_abad[0],
    ...
    float target_tau[12] = {_sharedMemory().robotToSim.spiCommand.tau_abad_ff[0],
    ...
    float kp_gain[12] = {_sharedMemory().robotToSim.spiCommand.kp_abad[0],
    ...
    float kd_gain[12] = {_sharedMemory().robotToSim.spiCommand.kd_abad[0],
    ...
    if(1)
    {
        ...
        jointNominalConfig.head(7)<<0, 0, 0.2, 1, 0.0, 0, 0.0;
        jointNominalConfig.tail(12)<<target_q[0],target_q[1],target_q[2],
                   target_q[3],target_q[4],target_q[5],
                    target_q[6],target_q[7],target_q[8],
                    target_q[9],target_q[10],target_q[11];

        jointVelocityTarget.tail(12)<<target_qd[0],target_qd[1],target_qd[2],
                target_qd[3],target_qd[4],target_qd[5],
                target_qd[6],target_qd[7],target_qd[8],
                target_qd[9],target_qd[10],target_qd[11];

        jointForce.tail(12)<<target_tau[0],target_tau[1],target_tau[2],
                target_tau[3],target_tau[4],target_tau[5],
                target_tau[6],target_tau[7],target_tau[8],
                target_tau[9],target_tau[10],target_tau[11];


        jointPgain.tail(12)<<kp_gain[0],kp_gain[1],kp_gain[2],
                kp_gain[3],kp_gain[4],kp_gain[5],
                kp_gain[6],kp_gain[7],kp_gain[8],
                kp_gain[9],kp_gain[10],kp_gain[11];
        jointDgain.tail(12)<<kd_gain[0],kd_gain[1],kd_gain[2],
                kd_gain[3],kd_gain[4],kd_gain[5],
                kd_gain[6],kd_gain[7],kd_gain[8],
                kd_gain[9],kd_gain[10],kd_gain[11];
    }
    else {
            jointNominalConfig << 0, 0, 0.2, 1, 0, 0, 0.0, 0.0, 0.3, -1, 0.0, 1, -2.3,
            0.0, 0.3, -1, 0.0, 1, -2.3;
    }
    panda5->setPdGains(jointPgain, jointDgain);

    panda5->setGeneralizedForce(jointForce);
...
int main(int argc, char* argv[]) {
    ...
    // 设置地形参数
    int xSamples = 500;  // x方向采样点数
    int ySamples = 500;  // y方向采样点数
    double xSize = 9.0;  // x方向地形的长度
    double ySize = 12.0;  // y方向地形的宽度

    // 创建高度图数组并初始化
    std::vector<double> heightValues(xSamples * ySamples, 0.0);

    // 定义楼梯的参数
    int steps = 30;// 楼梯的级数
    int stepSampleWidth = xSamples / steps;  // 每级台阶的采样点数
    double stepHeight = 0.15; // 每级台阶的高度

    // 设置每级台阶的高度
    for (int i = 0; i < steps; ++i) {
        for (int x = i * stepSampleWidth; x < (i + 1) * stepSampleWidth; ++x) {
            for (int y = 0; y < ySamples; ++y) {
                heightValues[y * xSamples + x] = i * stepHeight;  // 确保同一级的高度相同
            }
        }
    }

    // 使用高度图创建地形
    auto heightMap = world.addHeightMap(xSamples, ySamples, xSize, ySize, 20.0,0.0, heightValues);
    ...
    jointPgain.tail(num_dof-6).setConstant(20.0);
    ...
    // raisim::ArticulatedSystem::setJ

    float _abadGearRatio = 16.227;
    float _hipGearRatio = 16.227;
    float _kneeGearRatio = 16.227;
    float _ankleRatio = 16.227;

    float _motorTauMax = 200.0;//200.0;
    float _batteryV = 48;//48;
    float _motorKT = 0.05;//0.281;//0.05f;//.05;  // this is flux linkage * pole pairs
    float _motorR = 0.173f;//0.4554;//0.173f;//0.173;//0.4554;//0.4554;//0.173;//0.4554;//0.173;//0.4554;//0.173;
    float _jointDamping = .01;
    float _jointDryFriction = .2;//123

    _actuatorModels.emplace_back(_abadGearRatio, _motorKT, _motorR, _batteryV,
                      _jointDamping, _jointDryFriction, _motorTauMax);
    _actuatorModels.emplace_back(_hipGearRatio, _motorKT, _motorR, _batteryV,
                      _jointDamping, _jointDryFriction, _motorTauMax);
    _actuatorModels.emplace_back(_kneeGearRatio, _motorKT, _motorR, _batteryV,
                      _jointDamping, _jointDryFriction, _motorTauMax);
    ...
    tau_now = panda5->getGeneralizedForce().e().tail(12);
  ...
  for (; ; ) {
    ...
    float target_qqq[12] = {_sharedMemory().robotToSim.spiCommand.q_des_abad[0],
                          _sharedMemory().robotToSim.spiCommand.q_des_hip[0],
                          _sharedMemory().robotToSim.spiCommand.q_des_knee[0],
                          _sharedMemory().robotToSim.spiCommand.q_des_abad[1],
                          _sharedMemory().robotToSim.spiCommand.q_des_hip[1],
