...
Mat3<T> coordinateRotation(CoordinateAxis axis, T theta) {
  ...
  if (axis == CoordinateAxis::X) {
    R << 1, 0, 0, 0, c, s, 0, -s, c;//Rot(x).tanspose---dogXP---
  } else if (axis == CoordinateAxis::Y) {
    R << c, 0, -s, 0, 1, 0, s, 0, c;//Rot(y).tanspose---dogXP---
...
Mat3<typename T::Scalar> crossMatrix(const Eigen::MatrixBase<T>& v) {
  ...
  Mat3<typename T::Scalar> m;////vec3---->skew symmetric matrix ---dogXP---
  m << 0, -v(2), v(1),
       v(2), 0, -v(0),
       -v(1), v(0), 0;
  return m;
...
Mat3<typename T::Scalar> rpyToRotMat(const Eigen::MatrixBase<T>& v) {
  ...
  return m;///----Rot(z)*Rot(y)*Rot(x) -----dogXP-----
}
...
Mat3<typename T::Scalar> quaternionToRotationMatrix(
  ...
  /*
   * ref osg
   * http://www.360doc.com/content/12/0331/09/8875560_199510506.shtml
   * q0 * q0 + q1 * q1 + q2 * q2 + q3 * q3  = 1.0
   */
