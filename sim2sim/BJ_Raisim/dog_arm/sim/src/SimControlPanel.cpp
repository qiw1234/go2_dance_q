...
//      _miniCheetahDebugLCM(getLcmUrl(255))
{
  ...
  std::cout<<getConfigDirectoryPath() + getDefaultUserParameterFileName()<<std::endl;
  try {
...
//  ui->miniCheetahButton->setCheckable(true);
//  ui->simulatorButton->setCheckable(true);
  if (ui->miniCheetahButton->isChecked()) {
    robotType = RobotType::Uvc_dog;
  } else {
  ...
  // get run type
  if (!ui->simulatorButton->isChecked()) {
    createErrorMessage(
  ...
  //_graphicsWindow->resize(800, 720);
  //_graphicsWindow->resize(800, 720);//robot 3D size
  _graphicsWindow->resize(2100, 1300);
...
void SimControlPanel::on_simulatorTable_cellChanged(int row, int column) {
  ...
  //printf("on_simulatorTable_cellChanged\n");
...
void SimControlPanel::on_robotTable_cellChanged(int row, int column) {
  ...
  //printf("on_robotTable_cellChanged\n");
...
//        //cout << "cellName = " << parameter.get(parameter._kind).d << endl;
//      }
...
void SimControlPanel::on_kickButton_clicked() {
  ...
  SVec<double> kickVelocity;////////kick velocity
