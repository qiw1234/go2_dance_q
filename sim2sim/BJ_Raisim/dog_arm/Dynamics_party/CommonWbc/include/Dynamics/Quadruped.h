...
#ifdef _USE_ARM
constexpr size_t num_act_joint = 12 + _DOF_ARM;
constexpr size_t num_q = 19 + _DOF_ARM;
constexpr size_t dim_config = 18 + _DOF_ARM;
constexpr size_t num_leg = 4;
constexpr size_t num_leg_joint = 3;
constexpr size_t num_arm_joint = _DOF_ARM;
#else
...
class Quadruped {
 public:
  ...
  T _wheelGearRatio;
  T _wheelLinkLength;
  SpatialInertia<T> _wheelInertia,_wheelRotorInertia;
  Vec3<T> _wheelLocation, _wheelRotorLocation;
#endif
#ifdef _USE_ARM
  Vec3<T> _armLocation[_DOF_ARM],_armRotorLocation[_DOF_ARM];
  SpatialInertia<T> _armInertia[_DOF_ARM],_armRotorInertia[_DOF_ARM];
  T _armGearRatio[_DOF_ARM];
#endif
