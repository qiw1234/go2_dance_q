...
class BJTUDance:
    ...
    def PutToNet(self):
        ...
        base_ang_vel_w = [self.shareinfo_feed.sensor_package.imu_wxyz[0],
                          self.shareinfo_feed.sensor_package.imu_wxyz[1], self.shareinfo_feed.sensor_package.imu_wxyz[2]]
        ...
        for i in range(4):  # LF RF LH RH
            # print("leg ",i,"joint ",self.shareinfo_feed.sensor_package.joint_q[reindex_feet1[i]][0])
            for j in range(3):
    ...
    def inference_(self):
        ...
        while True:
            ...
            # print("self.actions_clip ",self.actions)
            default_dof_pos = [0.1,0.8,-1.5,  -0.1,0.8,-1.5,  0.1,1.,-1.5,  -0.1,1.,-1.5, 0,0,0, 0,0,0, 0,0]  # LF RF LH RH
            for i in range(4):
                for j in range(3):
                    self.joint_qd[reindex_feet1[i]][j] = self.actions_scaled.tolist()[i*3+j] + default_dof_pos[i*3+j]
                    self.joint_qd2[reindex_feet1[i]][j] = self.actions2_scaled.tolist()[i*3+j] + default_dof_pos[i*3+j]
                    
            for i in range(self.num_acts-12):
                self.joint_arm_d[i] = self.actions_scaled.tolist()[12 + i] + default_dof_pos[12 + i] 
                self.joint_arm_d2[i] = self.actions2_scaled.tolist()[12 + i] + default_dof_pos[12 + i] 
