...
class Tsinghua:
    ...
    def __init__(self):
        ...
        self.stand_height = 0.54
        self.episode_length_buf = torch.zeros(1, device=self.device)
        self.lock = threading.Lock()
        self.actions_last = torch.zeros(12,device=self.device)
        self.actions_last2 = torch.zeros(12,device=self.device)
        ...
        self.actor_state = torch.zeros(size = (512,), device=self.device)
        self.actions = torch.zeros(size = (12,), device=self.device)
        self.euler = 0

        self.actor_state2 = torch.zeros(size = (512,), device=self.device)
        self.actions2 = torch.zeros(size = (12,), device=self.device)
        
        self.shmaddr,self.semaphore,self.shmid = getsharememory_twodogs.CreatShareMem()##change
    ...
    def handler(self):
        print("收到SIGINT信号,ShareMemClose")
        getsharememory_twodogs.ShareMemClose(self.shmaddr,self.semaphore,self.shmid)
    ...
    def loadPolicy(self):
        self.remodel_0 = ModelLoad(device=self.device)
        self.remodel_1 = ModelLoad(device=self.device)
        self.remodel_2 = ModelLoad(device=self.device)
        self.remodel_3 = ModelLoad(device=self.device)
        self.remodel_4 = ModelLoad(device=self.device)
    ...
    def PutToNet(self):
        ...
        quaternion = [x,y,z,w]
        quat_tensor = torch.tensor([quaternion], dtype=torch.float32, device=self.device)
        vector = torch.tensor([[0, 0, -1]], dtype=torch.float32, device=self.device)
