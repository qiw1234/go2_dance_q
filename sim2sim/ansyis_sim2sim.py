...
isaacgym_torques = np.loadtxt('BJ_Raisim/net/HSW/data/wave_torque.csv', delimiter=",")
raisim_torques = np.loadtxt('BJ_Raisim/net/HSW/data/torques.csv', delimiter=",")

index = 3000

isaacgym_torques = isaacgym_torques[:index, :]
raisim_torques = raisim_torques[:index, :]

raisim_ang_vel = raisim_obs[:, 0:3]
raisim_projected_gravity = raisim_obs[:, 3: 6]
raisim_dof_pos = raisim_obs[:index, 6:24]
raisim_actions = raisim_obs[:index, 24:42] * 0.25 + default_dof_pos - raisim_dof_pos

isaacgym_ang_vel = isaacgym_obs[:, 0:3]
isaacgym_projected_gravity = isaacgym_obs[:, 3: 6]
isaacgym_dof_pos = isaacgym_obs[:index, 6:24]
isaacgym_actions = isaacgym_obs[:index, 24:42] * 0.25 + default_dof_pos - isaacgym_dof_pos
